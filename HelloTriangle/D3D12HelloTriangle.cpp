...
void D3D12HelloTriangle::OnInit()
{
	...
	LoadPipeline();
	LoadAssets();
	// Check for ray tracing capabilities on the device
	CheckRaytracingSupport();

	// Setup the acceleration structures (AS) for raytracing. When setting up geometry,
	// each bottom-level AS has its own transform matrix.
	...
	// and to their root signatures, defining the amount of memory carried by the rays (ray payload)
	CreateRayTracingPipeline(); // #DXR

	// #DXR Extra: Per-Instance Data
	// Create a constant buffer, with a color for each vertex of the triangle, for eachtriangle instance
	...
	// Create the buffer containing the raytracing result (always output in a UAV), and create the heap referencing the resources used by the raytracing,
	// such as the acceleration structure
	CreateShaderResourceHeap();

	// Create the shader binding table and idicating which shaders
...
void D3D12HelloTriangle::LoadAssets()
{
	...
	{
		// #DXR Extra: Per Instance
		// Create the vertex buffer for ground plane, similiarly to the triangle definition above
		CreatePlaneVB();
	}
...
void D3D12HelloTriangle::PopulateCommandList()
{
	...
	if (m_raster)
	{
		...
		// #DXR Extra: Per-Instance
		// In a way similiar to triangle rendering, rasterize the plane
	...
	else
	{
		...
		// On the last frame, the raytracing output was used as a copy source, 
		// to copy its contents into the render target. now we need to transition it to
		// a UAV so that the shaders can write to it
		...
		// all the SBT entries of a given type have the same size to allow a fixed stride

		//The ray generation shaders are alwsys at the beginning of the SBT
		...
		// The hit groups section start after the miss shaders. in this sampled
		// we have 1 hit group for the triangle
		...
		// We can then do the actual copy, before transitioning to the render target
...
// buffers in GPU memory along with their vertex count. The build is then done in 3
// steps: gathering geometry, computing the size of the required buffers, and building the actual AS
...
// Similarly to the bottom-level AS generation, it is done in 3 steps: gathering the
// instances, computing the memory requirments for the AS, and buiilding the AS itself
...
void D3D12HelloTriangle::CreateAccelerationStructures()
{
	...
	// Three instances for the triangle + 1 plane
	m_instances = { {bottomLevelBuffers.pResult, XMMatrixIdentity()},
					{bottomLevelBuffers.pResult, XMMatrixTranslation(-0.6f, 0, 0)},
					{bottomLevelBuffers.pResult, XMMatrixTranslation(0.6f, 0, 0)},
					// #DXR Extra: Per-Instance data
	...
	// Store the AS buffers. The rest of the buffer will be released once we exit the function
...
void D3D12HelloTriangle::CreateShaderBindingTable()
{
	...
	// The pointer to the beginning of the heap is the only paramenter required by shaders without root parameters
	D3D12_GPU_DESCRIPTOR_HANDLE srvUavHeapHandle = m_srvUavHeap->GetGPUDescriptorHandleForHeapStart();
	// The helper treats both root parameter pointer and heap pointer as void*,
	// while DX12 uses
...
//------------------------------------------
//
// The camera buffer is a constant buffer that stored the transform of 
// the camera, for use by both the rasterization and ray tracing. This method allocates the
// buffer where the matricies will be copied. For the sake of code
// clarity, it also creates a heap containing this buffer, to use in the rasterization path
//
...
//--------------------------------------------
//
// Creates and copies the view, model and projection matricies of the camera
//
...
void D3D12HelloTriangle::CreatePlaneVB()
{
  ...
  // actually transfer.
	...
	// Copy the triangle data to the vertex buffer
	UINT8* pVertexDataBegin;
	CD3DX12_RANGE readRange(0, 0); // We do not indent to read from this resource on the CPU
...
{
	// Due to HLSL packing rules, we create the CB with 9 float4 (each needs to start on a 16 byte boundry)
	XMVECTOR bufferData[] = {
		// A
